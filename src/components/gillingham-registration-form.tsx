...
/**
 * Darul-ilm KMWA (Gillingham) student registration form.
 *
 * Field names below intentionally match the original hosted form
 * (https://darulilmkent.ibesystems.uk/api/application-form/?form=only)
 * exactly, because the backend at IBEUK_ENDPOINT expects those names.
 * This is a separate school/page from the Chatham RegistrationForm —
 * same shared backend, different `h`/`t` identifiers and content.
 *
...
const CAPTCHA_ORDER = ["3", "1", "2", "5", "4"] as const; // display order shown to user (Kent page order)
...
const GROUP_OPTIONS = ["Weekend Madrasa (Saturday & Sunday, 9:45am - 1pm)"];
...
export function GillinghamRegistrationForm() {
  ...
  function buildBody(): URLSearchParams {
    ...
    params.append("h", "rMx_ZGFydWxpbG1rZW50LmliZXN5c3RlbXMudWs=");
    params.append("t", "DarulilmKMWA");
    ...
    <div className="panel-card mx-auto mt-10 max-w-3xl p-6 md:p-8">
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className={alertInfoClass}>
          <p className="font-semibold text-primary">Darul-ilm KMWA</p>
          <p>Location: Gillingham Masjid</p>
          <p className="mt-2">
            Fees per month:
            <br />
            One child: £50
            <br />
            Two children: £90 (£45 per child)
            <br />
            Three children: £135 (£45 per child)
            <br />
            Four children: £180 (£45 per child)
          </p>
        </div>
        ...
        <div className={alertInfoClass}>
          <p className="font-semibold text-primary">For further enquiries:</p>
          <p>Please send a message via WhatsApp on: 07778020745</p>
        </div>
        ...
        <label className="flex items-start gap-2 text-sm text-foreground">
          ...
          />
          <span>
            I consent that IBEUK can process this form and submit directly to Darul-ilm KMWA. Your
            data will not be stored nor used by us.
          </span>
        </label>
